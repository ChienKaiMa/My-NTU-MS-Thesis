...
In this chapter, we will review some major concepts about
quantum state discrimination.

We follow the convention of quantum communication discussions, which involves
the hypothetical individuals: the message sender Alice, the message receiver
Bob, and the eavesdropper Eve. For the scenarios discussed in this thesis, we
assume there are no eavesdroppers. If needed, readers can go to Section 2.2.4
and Section 12.1 of \cite{qcqi} or Chapter 3 of \cite{10.5555/3240076} for
more details.

In short, Alice prepares one of the quantum states from a fixed set of quantum
states which is known to Alice and Bob, and Bob will perform measurement to
determine which one of the quantum states is sent by Alice \cite{qcqi}.
Non orthogonal states can't be reliably discriminated; in other
words, there does not exist a measurement such that every state in the set
of non-orthogonal states known by Alice and Bob, can be correctly identified
with probability 1 \cite{qcqi}.

\section{The quantum state discrimination problem}
Let $S_{\ket{\psi}} \equiv \{\ket{\psi_{1}}, \ket{\psi_{2}}, \dots, \ket{\psi_{l}}\}$
be a set of quantum states, known to Alice and Bob before communication.
In the discussions of state discrimination, the states are usually
non-orthogonal, that is,
\begin{align}
...
$S_{prior}$, the quantum state discrimination problem stimulates discussions
about the pros and cons of different ways to measure and identify the quantum
state prepared by Alice.
There are an abundance of figures of merit proposed (cite).
The most common figure of merit is minimizing the measurement error or
maximizing the probability of success discrimination.

Let us introduce different methods with an example.
\vspace*{0.2cm}

Let $l = 2$, $\ket{\psi_{1}} = \ket{0}$,
$\ket{\psi_{2}} = \dfrac{\ket{0}+\ket{1}}{\sqrt{2}}$, and
$S_{prior} = \{p_{1}, p_{2}\} = \{0.5, 0.5\}$.
We can verify that the two states are non-orthogonal since
$\braket{\psi_{1}}{\psi_{2}} = \dfrac{1}{\sqrt{2}} \neq 0$.

\section{Projective measurement}

The projective measurement is a set of $M$ measurement operators, denoted by
$S_{\Pi_{proj}} \equiv \{\Pi_{1}, \Pi_{2}, \dots, \Pi_{M}\}$
such that $\Sigma_{i = 1}^{M}\Pi_{i} = I, \Pi_{i}\Pi_{j} = \delta_{ij}, \forall \; 1 \leq i, j \leq M,$
where $\delta_{ij}$ is the Kronecker delta function.

A trivial projective measurement for the example states is
\begin{align}
     & \Pi_{1} = \ket{0}\bra{0}
...
We can evaluate the probability of success $P_{succ}$ as follows:
\begin{align}
     & \bra{\psi_{1}} \Pi_{1} \ket{\psi_{1}} = 1,
    \bra{\psi_{1}} \Pi_{2} \ket{\psi_{1}} = 0                   \\
     & \bra{\psi_{2}} \Pi_{1} \ket{\psi_{2}} = 0.5,
    \bra{\psi_{2}} \Pi_{2} \ket{\psi_{2}} = 0.5                 \\
     & P_{succ} = p_{1} \bra{\psi_{1}} \Pi_{1} \ket{\psi_{1}} +
    p_{2} \bra{\psi_{2}} \Pi_{2} \ket{\psi_{2}}
    = 0.5 \times 1 + 0.5 \times 0.5 = 0.75
\end{align}

Since Bob sometimes thinks he measures $\ket{\psi_{1}}$ when he actually
receives $\ket{\psi_{2}}$, we call this unwanted behavior "measurement error".
With this projective measurement, the measurement error only attributes to
the measurement error of $\ket{\psi_{2}}$.
