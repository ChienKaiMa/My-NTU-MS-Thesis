...
\begin{figure}[htbp]
    \centering
    \scalebox{0.3}{
        \includegraphics{figures/povm_pvm.png}
    }
...
Quantum circuits provide a universal framework for quantum computation,
enabling a diverse set of tasks including variational quantum algorithms (VQA)
\cite{peruzzo2014variational}, Grover’s search algorithm \cite{grover1996fast},
and Shor’s factoring algorithm \cite{shor1994algorithms}.
In the gate model, quantum circuits are constructed from sequences of unitary
gates, such as Hadamard, CNOT, and single-qubit rotations, applied to qubits
to perform computations or measurements.
These gates manipulate quantum states in a controlled manner, forming the
basis for implementing algorithms or measurement schemes, such as those used
for state discrimination tasks in subsequent chapters.
The universality of the gate model ensures that any quantum computation can be
approximated with a suitable sequence of gates, making it a cornerstone of
quantum information processing.

While quantum circuits can be executed on various platforms, including
ion-trap or photonic systems, we focus on IBM Quantum backends, typically
superconducting quantum processors with constrained qubit connectivity, using
the Qiskit framework for simulation and execution \cite{qiskit2023}.
Qiskit facilitates circuit design, simulation, and deployment on noisy
hardware environments, where errors like decoherence impact performance.
Transpilation maps high-level circuit designs to the backend’s basis gates
(e.g., CX, RZ), optimizing gate sequences to account for connectivity
constraints and minimize errors.
This process is critical for reliable execution of computational tasks, as
explored in later sections for specific applications like state discrimination.
