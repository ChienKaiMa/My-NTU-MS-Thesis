...
At each iteration (for example, iteration $k$), SCS solves the following set of linear equations:
\begin{align}
...
. The presence of the diagonal scaling $R \in \mathbf{R}^{(n+m) \times (n+m)}$ matrix is explained
in Non-identity DR Scaling. The $R_y$ term is negated to make the matrix quasi-definite; we can recover
...
solver which is included Intel oneAPI MKL library (cite).
https://www.cvxgrp.org/scs/linear\_solver/index.html
...
Pardiso tends to be faster than AMD / QDLDL, especially for larger problems. Since we are running our experiments
with Intel CPUs, it is very suitable for our needs.

We have also considered using GPU acceleration in SCS. However, since the developers claimed that cuDSS
and its integration are not well battle-tested as the other existing solvers, we leave it for future improvements.
