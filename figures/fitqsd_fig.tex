\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        \begin{axis}[
                xlabel={SDP Input Noise Parameter $\lambda_{\text{SDP}}$},
                ylabel={Success Probability $P_{\text{succ}}$},
                xmode=log,
                xmin=1e-6, xmax=1e-1,
                ymin=0.76, ymax=0.82,
                grid=major,
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Success probability $P_{\text{succ}}$ versus SDP input noise $\lambda_{\text{SDP}}$ for FitQSD methods on two-qubit entangled states with parameters $a_1 = 0.2$, $a_2 = 0.5$, $a_3 = 0.7$ (Subsection \ref{subsec:entangled_states}), priors $1/3$, using MOSEK at $\epsilon = 10^{-9}$. MinSS yields higher $P_{\text{succ}}$ but higher $L_2$ norm at low $\lambda_{\text{SDP}}$ due to precision limitations.}
